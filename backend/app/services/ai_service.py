...
async def generate_summary(entry_id: UUID, db: Session):
    ...
    try:
        ...
        if entry.content_type == "link":
            content = entry.content or entry.entry_metadata.get("description", "") if entry.entry_metadata else ""
            ...
            repo_desc = entry.entry_metadata.get("description", "") if entry.entry_metadata else ""
