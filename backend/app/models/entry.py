...
class Entry(Base):
    ...
    entry_metadata = Column(JSONB, name="metadata")  # Flexible metadata storage (using name to keep DB column name)
    ai_summary = Column(Text)
    summary_status = Column(String(20), default="pending", index=True)  # 'pending', 'processing', 'completed', 'failed'
    ...
    def __repr__(self):
        return f"<Entry(id={self.id}, title={self.title[:50]}, type={self.content_type})>"
